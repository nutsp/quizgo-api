...
var (
	...
	ErrExamSetQuestionLimitExceeded   = New("EXAM_SET_QUESTION_LIMIT_EXCEEDED", "จำนวนคำถามเกินจำนวนที่กำหนดไว้ในชุดข้อสอบ", http.StatusBadRequest)
	ErrAutoAssignQuestionsUnavailable = New("AUTO_ASSIGN_QUESTIONS_UNAVAILABLE", "คำถามในคลังไม่เพียงพอตามโครงสร้างชุดข้อสอบ", http.StatusBadRequest)
	ErrExamSetQuestionRulesRequired   = New("EXAM_SET_QUESTION_RULES_REQUIRED", "กรุณากำหนดโครงสร้างชุดข้อสอบก่อน", http.StatusBadRequest)
	ErrQuestionOutsideExamSetRules    = New("QUESTION_OUTSIDE_EXAM_SET_RULES", "คำถามอยู่นอกโครงสร้างชุดข้อสอบ", http.StatusBadRequest)
